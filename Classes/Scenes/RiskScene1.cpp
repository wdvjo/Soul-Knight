...
static void problemLoading(const char* filename)
{
	...
	printf("Depending on how you compiled you might have to add 'Resources/' in front of filenames in RiskScene1.cpp\n");
}

// on "init" you need to initialize your instance
bool RiskScene1::init(int weaponID, int curH, int curS, int curM)
{
	//////////////////////////////
	// 1. super init first
	if (!Scene::init())
	...
	CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("Approaching Forces.mp3", true);
	...
	keyListener->onKeyPressed = CC_CALLBACK_2(RiskScene1::onKeyPressed, this);

	keyListener->onKeyReleased = CC_CALLBACK_2(RiskScene1::onKeyReleased, this);
...
void RiskScene1::onKeyPressed(EventKeyboard::KeyCode keycode, Event *event)
{
	if (keycode == EventKeyboard::KeyCode::KEY_W) {
		...
		hero->attack(this, playerBullets, enemysList, 1);
...
void RiskScene1::onKeyReleased(EventKeyboard::KeyCode keycode, Event *event)
{
...
void RiskScene1::update(float delta)
{
	nowTime++;
...
void RiskScene1::getEnemysPoint(int SX, int SY)
{
	if(map == NULL)
	{
		return;
...
void RiskScene1::attackEnemys()
{
	for (auto iter = enemysList.begin(); iter != enemysList.end(); iter++)
	{
		...
		if (is_attack == 0)
		{
			eP->attack(this, enemyBullets, 1);
		}

	}
}

void RiskScene1::check()
{
	auto visibleSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
...
Hero* RiskScene1::getHero()
{
	return hero;
}

void RiskScene1::setHero(Hero* h)
{
	hero = h;
}

void RiskScene1::next()
{
	float curX = hero->curX;
	float curY = hero->curY;
	...
	if (tileGid != 0)
	{
		auto nextScene = RiskScene2::create(hero->getW1()->getID(), hero->getCH(), hero->getCS(), hero->getCM());

		Director::getInstance()->replaceScene(nextScene);
	}
}

void RiskScene1::closeScene()
{
...
RiskScene1* RiskScene1::create(int weaponID, int curH, int curS, int curM)
{
	RiskScene1 *pScene = new RiskScene1;
