...
static void problemLoading(const char* filename)
{
	...
	printf("Depending on how you compiled you might have to add 'Resources/' in front of filenames in RiskScene3.cpp\n");
}

// on "init" you need to initialize your instance
bool RiskScene3::init(int weaponID, int curH, int curS, int curM)
{
	//////////////////////////////
	// 1. super init first
	if (!Scene::init())
	...
	CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("Mabanua - Megalobox.mp3", true);

	auto visibleSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();

	map = CCTMXTiledMap::create("map3.tmx");
	...
	keyListener->onKeyPressed = CC_CALLBACK_2(RiskScene3::onKeyPressed, this);

	keyListener->onKeyReleased = CC_CALLBACK_2(RiskScene3::onKeyReleased, this);
...
void RiskScene3::onKeyPressed(EventKeyboard::KeyCode keycode, Event *event)
{
	if (keycode == EventKeyboard::KeyCode::KEY_W) {
		...
		hero->attack(this, playerBullets, enemysList, 3);
...
void RiskScene3::onKeyReleased(EventKeyboard::KeyCode keycode, Event *event)
{
...
void RiskScene3::update(float delta)
{
	nowTime++;
...
void RiskScene3::getEnemysPoint(int SX, int SY)
{
	if (map == NULL)
	{
		return;
	...
	for (int i = 1; i <= 1; i++)
	{
		auto Point = objects->getObject("boss");

		int X = Point.at("x").asInt();

		int Y = Point.at("y").asInt();

		Enemy* p = new Boss(X, Y);
...
void RiskScene3::attackEnemys()
{
	for (auto iter = enemysList.begin(); iter != enemysList.end(); iter++)
	{
		...
		int is_attack = rand() % 20;

		if (is_attack == 0)
		{
			eP->attack(this, enemyBullets, 3);
		}

	}
}

void RiskScene3::check()
{
	auto visibleSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
...
Hero* RiskScene3::getHero()
{
	return hero;
}

void RiskScene3::setHero(Hero* h)
{
	hero = h;
}

void RiskScene3::next()
{
	float curX = hero->curX;
	float curY = hero->curY;
...
RiskScene3* RiskScene3::create(int weaponID, int curH, int curS, int curM)
{
	RiskScene3 *pScene = new RiskScene3;
